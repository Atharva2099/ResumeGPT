...
window.askQuestion = async function() {
    ...
    try {
        ...
        if (response.ok) {
            chatMessages.innerHTML = chatMessages.innerHTML.replace(`<p><strong>AI:</strong> Thinking...</p>`, `<p><strong>AI:</strong> ${result.response}</p>`);
        } else {
            throw new Error(result.error || 'An error occurred while generating the response.');
        }
    } catch (error) {
